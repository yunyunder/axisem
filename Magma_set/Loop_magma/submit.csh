...
if ( $simtype == 'moment' ) then
    ...
    cp $homedir/STATIONS .
    cp $homedir/magma.sph .
    #cp $homedir/com_abs.sph .
    #cp $homedir/com_mit2s40.sph .
    #cp $homedir/tw_abs.sph .
endif
...
foreach isim (${srcapp})
    ...
    if ( $newqueue == 'true' ) then 
        ...
        ######## TORQUE/MAUI SCHEDULER #######
            ...
            # specific nodes name 
            #echo "#PBS -l nodes=${nodes}:ppn=20" >> run_solver.pbs
            echo "#PBS -l nodes=node07:ppn=20+node06:ppn=20+node05:ppn=20+node04:ppn=20+node03:ppn=20+node02:ppn=20+node01:ppn=20" >> run_solver.pbs
            #echo "#PBS -l nodes=node01:ppn=20+node02:ppn=20+node03:ppn=20+node05:ppn=20" >> run_solver.pbs
            #echo "#PBS -l nodes=node01:ppn=20+node02:ppn=20+node03:ppn=16" >> run_solver.pbs
            #echo "#PBS -l nodes=node02:ppn=2" >> run_solver.pbs
            echo "#PBS -l walltime=12:00:00" >> run_solver.pbs
            echo "#PBS -o $PWD/test.log" >> run_solver.pbs
            #echo "ulimit -s unlimited " >> run_solver.pbs
            echo "cd $PWD " >> run_solver.pbs
            echo "$mpiruncmd -np ${nodnum} $PWD/axisem  > $outputname " >> run_solver.pbs
            qsub run_solver.pbs
